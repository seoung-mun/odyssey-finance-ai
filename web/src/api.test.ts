...
describe("API client", () => {
  it("sends the in-memory token and does not retry a 401", async () => {
    ...
    await expect(client.get("/dashboard")).rejects.toMatchObject({ status: 401 });
    expect(fetcher).toHaveBeenCalledTimes(1);
    expect((fetcher.mock.calls[0][1].headers as Headers).get("Authorization")).toBe("Bearer access-only-in-memory");
  ...
  it("preserves requestId from a 500 problem response", async () => {
    ...
    await expect(client.get("/dashboard")).rejects.toEqual(
