...
구현 기준은 `API/openapi-internal.yaml`이다. 기획서는 블록 부트스트랩을 적어두었지만
현재 OpenAPI는 IID 부트스트랩을 요구하므로 현재 코드도 IID를 사용한다. 두 방식의
비교와 평가 방법은 `docs/IID-vs-블록-부트스트랩.md`에 따로 정리했다.
...
현재 평균과 사용자 입력이 모두 0원이면 계산할 수 있다. 현재 평균이 0원인데
...
월저축 = 현재 월평균 지출 - 줄인 월지출
누적저축 = 1개월차부터 해당 달까지의 월저축 합계
```

이미 지출한 이번 달 금액은 전체 누적저축에서 뺀다. 예정지출은 해당 달부터 마지막
달까지의 누적값에 계속 영향을 준다.
...
| 계산에 필요한 이력이 부족함 | `INSUFFICIENT_HISTORY` 422 |
| 기간이 0개월 이하 | `INVALID_HORIZON` 422 |
...
### 8-2. 기획서는 블록, OpenAPI는 IID로 다른 문제

실제 Spring과의 호출 계약인 OpenAPI를 우선해 IID로 구현했다. 후보를 즉시 바꾸지
않고 나중에 같은 데이터와 평가 지표로 IID·2개월 블록·3개월 블록을 비교한다.
...
현재 Python 자동 테스트는 46개다. `TestClient`를 실행하면 Starlette가 HTTPX2로 이전하라는
...
두 방식의 성능 비교 전까지는 IID를 유지한다. 평가 기준은
`docs/IID-vs-블록-부트스트랩.md`를 따른다.
...
### 10-3. 지나치게 큰 `aggressiveWarningPct`

`aggressiveWarningPct=10**1000`처럼 현실적이지 않은 큰 정수가 오면 현재 유한수 검사에서
`OverflowError`가 발생할 수 있다. 잘못된 정책값이므로 최종적으로는 422로 반환해야 한다.

### 10-4. 경로 수와 기간의 상한

`nPaths × horizonMonths`가 지나치게 크면 메모리가 부족해질 수 있다. 현재는 내부 토큰으로
보호되지만 잘못된 서버 설정까지 막지는 못한다. 제품 상한을 확정한 뒤 OpenAPI와
Pydantic에 같은 값을 적용해야 한다.
